...
export default function MeetingsPage() {
    ...
    const upcomingMeetings = meetings.filter(m => new Date(m.date) > new Date()).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    const highlightMeeting = upcomingMeetings[0];
                        ...
                        <div className="relative p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8">
                            <div className="space-y-4 text-center md:text-left">
                                <span className="inline-block px-3 py-1 rounded-full bg-white/20 text-sm font-medium backdrop-blur-sm">
                                    Major Upcoming Meeting
                                </span>
                                <h2 className="text-3xl md:text-5xl font-bold tracking-tight">{highlightMeeting.title}</h2>
                                <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 text-lg text-white/90">
                                    <span className="flex items-center gap-2">
                                        ...
                                        {new Date(highlightMeeting.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
                                    </span>
                                    <span className="flex items-center gap-2">
                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                        {highlightMeeting.startTime || 'Time TBD'}
                                    </span>
                                    <span className="flex items-center gap-2">
                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                        {highlightMeeting.venue}
                                    </span>
                                </div>
                            </div>
                            <div className="flex-shrink-0">
                                <Link href={`/dashboard/meetings/${highlightMeeting.id}`} className="btn bg-white text-blue-600 hover:bg-gray-100 border-none px-8 py-3 text-lg shadow-lg">
                                    View Details
                                </Link>
                            </div>
                        </div>
